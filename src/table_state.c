                                               ...
                                               struct imm_sequence_table const* table)
{
    ...
    struct imm_state_funcs funcs = {table_state_lprob, table_state_min_seq,
                                    table_state_max_seq};
    state->interface = imm_state_create(name, imm_sequence_table_get_abc(table), funcs, state);
    return state;
}
...
void imm_table_state_destroy(struct imm_table_state const* state)
{
    ...
    imm_sequence_table_destroy(state->table);
    free_c(state);
}
...
static double table_state_lprob(struct imm_state const* state, struct imm_seq const seq)
{
    ...
    return imm_sequence_table_lprob(s->table, seq);
}
