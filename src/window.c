...
struct imm_subseq imm_window_get(struct imm_window const* window, uint16_t index)
{
    ...
    uint16_t const length = (uint16_t)MIN((uint_fast32_t)window->length,
                                          (uint_fast32_t)imm_seq_length(window->seq) - offset);
...
uint16_t imm_window_size(struct imm_window const* window)
{
    ...
    do {
        ...
        length = (uint16_t)MIN((uint_fast32_t)window->length,
                               (uint_fast32_t)imm_seq_length(window->seq) - poffset);
        ++size;
