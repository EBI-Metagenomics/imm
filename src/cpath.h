...
static inline bool         cpath_valid(struct cpath const* cpath, uint_fast32_t row, unsigned state);
static inline uint_fast8_t cpath_get_len(struct cpath const* cpath, uint_fast32_t row, unsigned state);
static inline unsigned     cpath_get_trans(struct cpath const* cpath, uint_fast32_t row, unsigned state);
static inline void         cpath_set_invalid(struct cpath* cpath, uint_fast32_t row, unsigned state);
static inline void         cpath_set_len(struct cpath* cpath, uint_fast32_t row, unsigned state, uint_fast8_t len);
static inline void         cpath_set_trans(struct cpath* cpath, uint_fast32_t row, unsigned state, unsigned trans);
int                        cpath_setup(struct cpath* path, uint_fast32_t len);

static inline uint8_t       __bits_len(struct cpath const* cpath, uint_fast32_t row, unsigned state);
static inline uint_fast64_t __bits_start(struct cpath const* cpath, uint_fast32_t row, unsigned state);
...
{
    uint_fast64_t start = __bits_start(cpath, row, state);
    return (unsigned)bitarr_get(cpath->bitarr, start, cpath->bits_trans[state]);
}
