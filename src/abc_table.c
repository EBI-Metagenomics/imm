#include "imm/abc_table.h"
#include "float.h"
#include "free.h"
#include <stdlib.h>
#include <string.h>

struct imm_abc_table const* imm_abc_table_create(struct imm_abc const* abc, imm_float const* lprobs)
{
    struct imm_abc_table* abc_table =
        malloc(sizeof(struct imm_abc_table) + sizeof(imm_float) * imm_abc_length(abc));
    abc_table->abc = abc;
    memcpy(abc_table->lprobs, lprobs, sizeof(*lprobs) * imm_abc_length(abc));
    return abc_table;
}

void imm_abc_table_destroy(struct imm_abc_table const* abc_table) { free_c(abc_table); }
