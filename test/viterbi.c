...
static imm_float single_viterbi(struct imm_hmm const *hmm,
...
void test_hmm_viterbi_one_mute_state(void)
{
    ...
    cass_close(single_viterbi(hmm, EMPTY, imm_mute_state_super(state), &path),
               imm_log(0.5));
    cass_close(imm_hmm_loglikelihood(hmm, EMPTY, path), imm_log(0.5));
    imm_path_destroy(path);

    cass_cond(
        !is_valid(single_viterbi(hmm, C, imm_mute_state_super(state), &path)));
...
void test_hmm_viterbi_two_mute_states(void)
{
    ...
    struct imm_path *path = NULL;
    cass_close(single_viterbi(hmm, EMPTY, imm_mute_state_super(state0), &path),
               imm_log(0.5));
    cass_close(imm_hmm_loglikelihood(hmm, EMPTY, path), imm_log(0.5));
    imm_path_destroy(path);
    ...
    cass_cond(!is_valid(
        single_viterbi(hmm, EMPTY, imm_mute_state_super(state1), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);

    imm_hmm_set_trans(hmm, imm_mute_state_super(state0),
                      imm_mute_state_super(state1), imm_log(0.1));

    cass_close(single_viterbi(hmm, EMPTY, imm_mute_state_super(state1), &path),
               imm_log(0.5) + imm_log(0.1));
    cass_close(imm_hmm_loglikelihood(hmm, EMPTY, path),
               imm_log(0.5) + imm_log(0.1));
    imm_path_destroy(path);
...
void test_hmm_viterbi_mute_cycle(void)
{
    ...
    imm_hmm_set_trans(hmm, imm_mute_state_super(state0),
                      imm_mute_state_super(state1), imm_log(0.2));
    imm_hmm_set_trans(hmm, imm_mute_state_super(state1),
                      imm_mute_state_super(state0), imm_log(0.2));

    struct imm_path *path = NULL;
    cass_cond(!is_valid(
        single_viterbi(hmm, EMPTY, imm_mute_state_super(state0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);
...
void test_hmm_viterbi_one_normal_state(void)
{
    ...
    struct imm_normal_state *state =
        imm_normal_state_create(0, "State0", abc, lprobs0);
    ...
    struct imm_path *path = NULL;
    cass_cond(!is_valid(
        single_viterbi(hmm, EMPTY, imm_normal_state_super(state), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, A, imm_normal_state_super(state), &path),
               imm_log(1.0) + imm_log(0.25));
    cass_close(imm_hmm_loglikelihood(hmm, A, path),
               imm_log(1.0) + imm_log(0.25));
    imm_path_destroy(path);

    cass_cond(!is_valid(
        single_viterbi(hmm, T, imm_normal_state_super(state), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, T, path)));
    imm_path_destroy(path);

    cass_cond(!is_valid(
        single_viterbi(hmm, AC, imm_normal_state_super(state), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, AC, path)));
    imm_path_destroy(path);

    imm_hmm_set_trans(hmm, imm_normal_state_super(state),
                      imm_normal_state_super(state), imm_log(0.1));
    ...
    cass_close(single_viterbi(hmm, AA, imm_normal_state_super(state), &path),
               imm_log(0.1) + 2 * imm_log(0.25));
    cass_close(imm_hmm_loglikelihood(hmm, AA, path),
               imm_log(0.1) + 2 * imm_log(0.25));
    imm_path_destroy(path);

    cass_cond(!is_valid(
        single_viterbi(hmm, ACT, imm_normal_state_super(state), &path)));
...
void test_hmm_viterbi_two_normal_states(void)
{
    ...
    struct imm_normal_state *state1 =
        imm_normal_state_create(1, "State1", abc, lprobs1);
    ...
    imm_hmm_set_trans(hmm, imm_normal_state_super(state0),
                      imm_normal_state_super(state1), imm_log(0.3));

    struct imm_path *path = NULL;
    cass_cond(!is_valid(
        single_viterbi(hmm, EMPTY, imm_normal_state_super(state0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, A, imm_normal_state_super(state0), &path),
    ...
    cass_cond(!is_valid(
        single_viterbi(hmm, T, imm_normal_state_super(state0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, T, path)));
    imm_path_destroy(path);

    cass_cond(!is_valid(
        single_viterbi(hmm, AC, imm_normal_state_super(state0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, AC, path)));
    imm_path_destroy(path);

    imm_float desired =
        imm_log(0.1) + imm_log(0.25) + imm_log(0.3) + imm_log(0.5);
    cass_close(single_viterbi(hmm, AT, imm_normal_state_super(state1), &path),
               desired);
    cass_close(imm_hmm_loglikelihood(hmm, AT, path), desired);
    imm_path_destroy(path);

    cass_cond(!is_valid(
        single_viterbi(hmm, ATT, imm_normal_state_super(state1), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, ATT, path)));
    imm_path_destroy(path);

    imm_hmm_set_trans(hmm, imm_normal_state_super(state1),
                      imm_normal_state_super(state1), imm_log(0.5));
    ...
    cass_close(single_viterbi(hmm, ATT, imm_normal_state_super(state1), &path),
               desired);
...
void test_hmm_viterbi_normal_states(void)
{
    ...
    imm_hmm_set_trans(hmm, imm_normal_state_super(state0),
                      imm_normal_state_super(state0), imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_normal_state_super(state0),
                      imm_normal_state_super(state1), imm_log(0.2));
    imm_hmm_set_trans(hmm, imm_normal_state_super(state1),
                      imm_normal_state_super(state1), imm_log(1.0));
    ...
    cass_cond(!is_valid(
        single_viterbi(hmm, EMPTY, imm_normal_state_super(state1), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);
    ...
    cass_cond(!is_valid(
        single_viterbi(hmm, A, imm_normal_state_super(state1), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, A, path)));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, AG, imm_normal_state_super(state0), &path),
               -3.178053830348);

    cass_close(imm_hmm_loglikelihood(hmm, AG, path), -3.178053830348);
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, AG, imm_normal_state_super(state1), &path),
               -3.295836866004);
    ...
    cass_cond(!is_valid(
        single_viterbi(hmm, AGT, imm_normal_state_super(state0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, AGT, path)));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, AGT, imm_normal_state_super(state1), &path),
               -4.106767082221);
    ...
    cass_cond(!is_valid(
        single_viterbi(hmm, AGTC, imm_normal_state_super(state0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, AGTC, path)));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, AGTC, imm_normal_state_super(state1), &path),
               -6.303991659557);
    cass_close(imm_hmm_loglikelihood(hmm, AGTC, path), -6.303991659557);
    imm_path_destroy(path);

    cass_equal(imm_hmm_set_trans(hmm, imm_normal_state_super(state0),
                                 imm_normal_state_super(state0), zero()),
               IMM_ILLEGALARG);
    cass_equal(imm_hmm_set_trans(hmm, imm_normal_state_super(state0),
                                 imm_normal_state_super(state1), zero()),
               IMM_ILLEGALARG);
    cass_equal(imm_hmm_set_trans(hmm, imm_normal_state_super(state1),
                                 imm_normal_state_super(state0), zero()),
               IMM_ILLEGALARG);
    cass_equal(imm_hmm_set_trans(hmm, imm_normal_state_super(state1),
                                 imm_normal_state_super(state1), zero()),
    ...
    imm_hmm_set_trans(hmm, imm_normal_state_super(state0),
                      imm_normal_state_super(state0), imm_log(0.9));
    ...
    cass_close(single_viterbi(hmm, AA, imm_normal_state_super(state0), &path),
...
void test_hmm_viterbi_profile1(void)
{
    ...
    struct imm_normal_state *M0 =
        imm_normal_state_create(3, "M0", abc, M0_lprobs);

    imm_float I0_lprobs[] = {imm_log(0.5), imm_log(0.5)};
    struct imm_normal_state *I0 =
        imm_normal_state_create(4, "I0", abc, I0_lprobs);
    ...
    imm_hmm_set_trans(hmm, imm_mute_state_super(start),
                      imm_mute_state_super(D0), imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_mute_state_super(D0), imm_mute_state_super(end),
                      imm_log(1.0));
    imm_hmm_set_trans(hmm, imm_mute_state_super(start),
                      imm_normal_state_super(M0), imm_log(0.5));
    imm_hmm_set_trans(hmm, imm_normal_state_super(M0),
                      imm_mute_state_super(end), imm_log(0.8));
    imm_hmm_set_trans(hmm, imm_normal_state_super(M0),
                      imm_normal_state_super(I0), imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_normal_state_super(I0),
                      imm_normal_state_super(I0), imm_log(0.2));
    imm_hmm_set_trans(hmm, imm_normal_state_super(I0),
                      imm_mute_state_super(end), imm_log(1.0));

    struct imm_path *path = NULL;
    cass_close(single_viterbi(hmm, EMPTY, imm_mute_state_super(end), &path),
    ...
    cass_close(single_viterbi(hmm, EMPTY, imm_mute_state_super(D0), &path),
               imm_log(0.1));
    cass_close(imm_hmm_loglikelihood(hmm, EMPTY, path), imm_log(0.1));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, EMPTY, imm_mute_state_super(start), &path),
               imm_log(1.0));
    cass_close(imm_hmm_loglikelihood(hmm, EMPTY, path), imm_log(1.0));
    imm_path_destroy(path);

    cass_cond(!is_valid(
        single_viterbi(hmm, EMPTY, imm_normal_state_super(M0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);

    cass_cond(
        !is_valid(single_viterbi(hmm, A, imm_mute_state_super(start), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, A, path)));
    imm_path_destroy(path);

    cass_cond(
        !is_valid(single_viterbi(hmm, A, imm_mute_state_super(D0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, A, path)));
    imm_path_destroy(path);

    cass_cond(
        !is_valid(single_viterbi(hmm, A, imm_normal_state_super(I0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, A, path)));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, A, imm_normal_state_super(M0), &path),
    ...
    cass_close(single_viterbi(hmm, A, imm_mute_state_super(end), &path),
    ...
    cass_close(single_viterbi(hmm, B, imm_normal_state_super(M0), &path),
               imm_log(0.5) + imm_log(0.2));

    cass_close(imm_hmm_loglikelihood(hmm, B, path),
               imm_log(0.5) + imm_log(0.2));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, B, imm_mute_state_super(end), &path),
    ...
    cass_cond(
        !is_valid(single_viterbi(hmm, AA, imm_normal_state_super(M0), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, AA, path)));
    imm_path_destroy(path);

    imm_float desired =
        imm_log(0.5) + imm_log(0.4) + imm_log(0.1) + imm_log(0.5);
    cass_close(single_viterbi(hmm, AA, imm_mute_state_super(end), &path),
               desired);
    ...
    desired = imm_log(0.5) + imm_log(0.4) + imm_log(0.1) + imm_log(0.2) +
              2 * imm_log(0.5);
    cass_close(single_viterbi(hmm, AAB, imm_mute_state_super(end), &path),
               desired);
...
void test_hmm_viterbi_profile2(void)
{
    ...
    struct imm_normal_state *M0 =
        imm_normal_state_create(1, "M0", abc, M0_lprobs);
    struct imm_normal_state *I0 =
        imm_normal_state_create(2, "I0", abc, ins_lprobs);

    struct imm_mute_state *D1 = imm_mute_state_create(3, "D1", abc);
    struct imm_normal_state *M1 =
        imm_normal_state_create(4, "M1", abc, M1_lprobs);
    struct imm_normal_state *I1 =
        imm_normal_state_create(5, "I1", abc, ins_lprobs);

    struct imm_mute_state *D2 = imm_mute_state_create(6, "D2", abc);
    struct imm_normal_state *M2 =
        imm_normal_state_create(7, "M2", abc, M2_lprobs);
    ...
    imm_hmm_set_trans(hmm, imm_mute_state_super(start),
                      imm_normal_state_super(M0), 0.0);
    imm_hmm_set_trans(hmm, imm_mute_state_super(start),
                      imm_normal_state_super(M1), 0.0);
    imm_hmm_set_trans(hmm, imm_mute_state_super(start),
                      imm_normal_state_super(M2), 0.0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M0),
                      imm_normal_state_super(M1), 0.0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M0),
                      imm_normal_state_super(M2), 0.0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M0),
                      imm_mute_state_super(end), 0.0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M1),
                      imm_normal_state_super(M2), 0.0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M1),
                      imm_mute_state_super(end), 0.0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M2),
                      imm_mute_state_super(end), 0.0);

    imm_hmm_set_trans(hmm, imm_normal_state_super(M0),
                      imm_normal_state_super(I0), imm_log(0.2));
    imm_hmm_set_trans(hmm, imm_normal_state_super(M0), imm_mute_state_super(D1),
                      imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_normal_state_super(I0),
                      imm_normal_state_super(I0), imm_log(0.5));
    imm_hmm_set_trans(hmm, imm_normal_state_super(I0),
                      imm_normal_state_super(M1), imm_log(0.5));

    imm_hmm_set_trans(hmm, imm_normal_state_super(M1), imm_mute_state_super(D2),
                      imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_normal_state_super(M1),
                      imm_normal_state_super(I1), imm_log(0.2));
    imm_hmm_set_trans(hmm, imm_normal_state_super(I1),
                      imm_normal_state_super(I1), imm_log(0.5));
    imm_hmm_set_trans(hmm, imm_normal_state_super(I1),
                      imm_normal_state_super(M2), imm_log(0.5));
    imm_hmm_set_trans(hmm, imm_mute_state_super(D1), imm_mute_state_super(D2),
                      imm_log(0.3));
    imm_hmm_set_trans(hmm, imm_mute_state_super(D1), imm_normal_state_super(M2),
                      imm_log(0.7));

    imm_hmm_set_trans(hmm, imm_mute_state_super(D2), imm_mute_state_super(end),
                      imm_log(1.0));

    struct imm_path *path = NULL;
    cass_close(single_viterbi(hmm, A, imm_normal_state_super(M2), &path),
               imm_log(0.05));
    cass_close(imm_hmm_loglikelihood(hmm, A, path), imm_log(0.05));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, B, imm_normal_state_super(M2), &path),
               imm_log(0.05));
    cass_close(imm_hmm_loglikelihood(hmm, B, path), imm_log(0.05));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, C, imm_normal_state_super(M2), &path),
               imm_log(0.05));
    cass_close(imm_hmm_loglikelihood(hmm, C, path), imm_log(0.05));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, D, imm_normal_state_super(M2), &path),
               imm_log(0.05));
    ...
    cass_close(single_viterbi(hmm, C, imm_mute_state_super(end), &path),
               imm_log(0.6));
    cass_close(imm_hmm_loglikelihood(hmm, C, path), imm_log(0.6));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, D, imm_mute_state_super(end), &path),
               imm_log(0.05));
    cass_close(imm_hmm_loglikelihood(hmm, D, path), imm_log(0.05));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, A, imm_normal_state_super(M1), &path),
               imm_log(0.6));
    cass_close(imm_hmm_loglikelihood(hmm, A, path), imm_log(0.6));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, C, imm_normal_state_super(M1), &path),
               imm_log(0.4));
    cass_close(imm_hmm_loglikelihood(hmm, C, path), imm_log(0.4));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, CA, imm_mute_state_super(end), &path),
               2 * imm_log(0.6));
    cass_close(imm_hmm_loglikelihood(hmm, CA, path), 2 * imm_log(0.6));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, CD, imm_normal_state_super(I0), &path),
               imm_log(0.6) + imm_log(0.2) + imm_log(0.7));
    cass_close(imm_hmm_loglikelihood(hmm, CD, path),
               imm_log(0.6) + imm_log(0.2) + imm_log(0.7));
    imm_path_destroy(path);

    imm_float desired = imm_log(0.6) + imm_log(0.2) + 3 * imm_log(0.7) +
                        3 * imm_log(0.5) + imm_log(0.6);
    cass_close(single_viterbi(hmm, CDDDA, imm_mute_state_super(end), &path),
               desired);
    ...
    cass_close(single_viterbi(hmm, CDDDAB, imm_mute_state_super(end), &path),
               desired);
    ...
    cass_close(single_viterbi(hmm, CDDDABA, imm_normal_state_super(M2), &path),
               desired);
    ...
    cass_close(single_viterbi(hmm, CDDDABA, imm_normal_state_super(M1), &path),
               desired);
    cass_close(imm_hmm_loglikelihood(hmm, CDDDABA, path), desired);
    imm_path_destroy(path);
    ...
    cass_close(single_viterbi(hmm, CDDDABA, imm_mute_state_super(end), &path),
               desired);
    cass_close(imm_hmm_loglikelihood(hmm, CDDDABA, path), desired);
    imm_path_destroy(path);
...
void test_hmm_viterbi_profile_delete(void)
{
    ...
    imm_hmm_set_trans(hmm, imm_normal_state_super(N0),
                      imm_normal_state_super(N1), imm_log(0.5));
    imm_hmm_set_trans(hmm, imm_normal_state_super(N0), imm_mute_state_super(M),
                      imm_log(0.5));
    imm_hmm_set_trans(hmm, imm_normal_state_super(N1),
                      imm_normal_state_super(N2), imm_log(0.5));
    imm_hmm_set_trans(hmm, imm_mute_state_super(M), imm_normal_state_super(N2),
                      imm_log(0.5));

    struct imm_path *path = NULL;
    cass_close(single_viterbi(hmm, A, imm_normal_state_super(N0), &path),
               imm_log(0.5));
    cass_close(imm_hmm_loglikelihood(hmm, A, path), imm_log(0.5));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, A, imm_mute_state_super(M), &path),
               2 * imm_log(0.5));
    cass_close(imm_hmm_loglikelihood(hmm, A, path), 2 * imm_log(0.5));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, AB, imm_normal_state_super(N2), &path),
               4 * imm_log(0.5));
...
void test_hmm_viterbi_global_profile(void)
{
    ...
    struct imm_normal_state *M0 =
        imm_normal_state_create(2, "M0", abc, M0_lprobs);

    imm_float M1_lprobs[] = {imm_log(0.01), imm_log(0.9), zero(), zero()};
    struct imm_normal_state *M1 =
        imm_normal_state_create(3, "M1", abc, M1_lprobs);

    imm_float M2_lprobs[] = {imm_log(0.5), imm_log(0.5), zero(), zero()};
    struct imm_normal_state *M2 =
        imm_normal_state_create(4, "M2", abc, M2_lprobs);
    ...
    struct imm_normal_state *I0 =
        imm_normal_state_create(8, "I0", abc, ins_lprobs);
    struct imm_normal_state *I1 =
        imm_normal_state_create(9, "I1", abc, ins_lprobs);
    ...
    imm_hmm_set_trans(hmm, imm_mute_state_super(start),
                      imm_normal_state_super(B), 0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(B), imm_normal_state_super(B),
                      0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(B),
                      imm_normal_state_super(M0), 0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(B),
                      imm_normal_state_super(M1), 0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(B),
                      imm_normal_state_super(M2), 0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M0),
                      imm_normal_state_super(M1), 0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M1),
                      imm_normal_state_super(M2), 0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M2), imm_mute_state_super(E),
                      0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M0), imm_mute_state_super(E),
                      0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M1), imm_mute_state_super(E),
                      0);

    imm_hmm_set_trans(hmm, imm_mute_state_super(E), imm_mute_state_super(end),
                      0);

    imm_hmm_set_trans(hmm, imm_mute_state_super(E), imm_normal_state_super(Z),
                      0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(Z), imm_normal_state_super(Z),
                      0);
    imm_hmm_set_trans(hmm, imm_normal_state_super(Z), imm_normal_state_super(B),
                      0);
    ...
    imm_hmm_set_trans(hmm, imm_mute_state_super(D1), imm_normal_state_super(M2),
                      0);
    imm_hmm_set_trans(hmm, imm_mute_state_super(D2), imm_mute_state_super(E),
                      0);
    ...
    struct imm_path *path = NULL;
    cass_cond(
        !is_valid(single_viterbi(hmm, C, imm_mute_state_super(start), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, C, path)));
    imm_path_destroy(path);
    ...
    cass_close(single_viterbi(hmm, CCA, imm_normal_state_super(B), &path),
               imm_log(0.01));
    cass_close(imm_hmm_loglikelihood(hmm, CCA, path), imm_log(0.01));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, CCA, imm_normal_state_super(M0), &path),
               imm_log(0.9));
    cass_close(imm_hmm_loglikelihood(hmm, CCA, path), imm_log(0.9));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, CCAB, imm_normal_state_super(M1), &path),
               2 * imm_log(0.9));
    cass_close(imm_hmm_loglikelihood(hmm, CCAB, path), 2 * imm_log(0.9));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, CCAB, imm_normal_state_super(I0), &path),
               imm_log(0.9 * 0.5 * 0.1));
    cass_close(imm_hmm_loglikelihood(hmm, CCAB, path),
               imm_log(0.9 * 0.5 * 0.1));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, CCABB, imm_normal_state_super(I0), &path),
               imm_log(0.9) + 2 * (imm_log(0.05)));
    cass_close(imm_hmm_loglikelihood(hmm, CCABB, path),
               imm_log(0.9) + 2 * (imm_log(0.05)));
    ...
    cass_close(single_viterbi(hmm, CCABA, imm_normal_state_super(M1), &path),
               desired);
    cass_close(imm_hmm_loglikelihood(hmm, CCABA, path), desired);
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, AA, imm_mute_state_super(D1), &path),
    ...
    cass_close(single_viterbi(hmm, AA, imm_mute_state_super(D2), &path),
               imm_log(0.01) + imm_log(0.9));
    ...
    cass_close(single_viterbi(hmm, AA, imm_mute_state_super(E), &path),
               imm_log(0.01) + imm_log(0.9));
    ...
    cass_close(single_viterbi(hmm, AAB, imm_normal_state_super(M2), &path),
               desired);
    cass_close(imm_hmm_loglikelihood(hmm, AAB, path), desired);
    imm_path_destroy(path);
...
void test_hmm_viterbi_table_states(void)
{
    ...
    imm_float N0_lprobs[] = {zero(), imm_log(0.5), imm_log(0.5), zero(),
                             zero()};
    struct imm_normal_state *N0 =
        imm_normal_state_create(3, "N0", abc, N0_lprobs);

    imm_float N1_lprobs[] = {imm_log(0.25), imm_log(0.25), imm_log(0.25),
                             imm_log(0.25), zero()};
    struct imm_normal_state *N1 =
        imm_normal_state_create(4, "N1", abc, N1_lprobs);
    ...
    cass_cond(imm_hmm_set_trans(hmm, imm_mute_state_super(S),
    ...
    imm_hmm_set_trans(hmm, imm_mute_state_super(S), imm_table_state_super(T),
                      imm_log(1.0));
    imm_hmm_set_trans(hmm, imm_table_state_super(T), imm_mute_state_super(D),
                      imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_table_state_super(T), imm_normal_state_super(N0),
                      imm_log(0.2));
    imm_hmm_set_trans(hmm, imm_mute_state_super(D), imm_normal_state_super(N1),
                      imm_log(0.3));
    imm_hmm_set_trans(hmm, imm_normal_state_super(N0),
                      imm_normal_state_super(N1), imm_log(0.4));
    imm_hmm_set_trans(hmm, imm_normal_state_super(N1), imm_mute_state_super(E),
                      imm_log(1.0));
    imm_hmm_set_trans(hmm, imm_mute_state_super(E), imm_normal_state_super(Z),
                      imm_log(0.5));
    imm_hmm_set_trans(hmm, imm_normal_state_super(Z), imm_normal_state_super(Z),
                      imm_log(2.0));
    imm_hmm_set_trans(hmm, imm_normal_state_super(Z), imm_table_state_super(T),
                      imm_log(0.6));

    struct imm_path *path = NULL;
    cass_cond(
        !is_valid(single_viterbi(hmm, TATX, imm_mute_state_super(E), &path)));
    cass_cond(!is_valid(imm_hmm_loglikelihood(hmm, TATX, path)));
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, TATA, imm_normal_state_super(N1), &path),
               -6.502290170873972);
    cass_close(imm_hmm_loglikelihood(hmm, TATA, path), -6.502290170873972);
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, TATA, imm_mute_state_super(E), &path),
               -6.502290170873972);
    cass_close(imm_hmm_loglikelihood(hmm, TATA, path), -6.502290170873972);
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, TATTX, imm_normal_state_super(Z), &path),
               -7.195437351433918);
    cass_close(imm_hmm_loglikelihood(hmm, TATTX, path), -7.195437351433918);
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, TATTXX, imm_normal_state_super(Z), &path),
               -6.502290170873972);
    cass_close(imm_hmm_loglikelihood(hmm, TATTXX, path), -6.502290170873972);
    imm_path_destroy(path);

    cass_close(single_viterbi(hmm, CAXCA, imm_mute_state_super(E), &path),
               -11.800607537422009);
...
void test_hmm_viterbi_cycle_mute_ending(void)
{
    ...
    imm_hmm_set_trans(hmm, imm_mute_state_super(start), imm_mute_state_super(B),
                      imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_mute_state_super(B), imm_normal_state_super(M),
                      imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_normal_state_super(M), imm_mute_state_super(E),
                      imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_mute_state_super(E), imm_mute_state_super(end),
                      imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_mute_state_super(E), imm_mute_state_super(J),
                      imm_log(0.1));
    imm_hmm_set_trans(hmm, imm_mute_state_super(J), imm_mute_state_super(B),
                      imm_log(0.1));
                                ...
                                struct imm_path **path)
{
    ...
    struct imm_task *task = imm_dp_task_create(dp);
