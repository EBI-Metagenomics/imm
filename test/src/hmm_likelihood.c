...
void test_hmm_likelihood_single_state(void)
{
    ...
    imm_path_append(path, imm_step_create(imm_normal_state_super(state), 1));
    imm_path_append(path, imm_step_create(imm_normal_state_super(state), 1));
    cass_cond(is_zero(imm_hmm_loglikelihood(hmm, AA, path)));
    imm_path_destroy(path);

    cass_cond(imm_hmm_normalize(hmm) == 0);
    cass_cond(imm_hmm_set_trans(hmm, imm_normal_state_super(state), imm_normal_state_super(state), zero()) == 0);
    cass_cond(imm_hmm_normalize(hmm) == 0);
    cass_cond(imm_hmm_set_trans(hmm, imm_normal_state_super(state), imm_normal_state_super(state), logf(0.5f)) == 0);
    ...
    imm_path_append(path, imm_step_create(imm_normal_state_super(state), 1));
    imm_path_append(path, imm_step_create(imm_normal_state_super(state), 1));
    cass_close(imm_hmm_loglikelihood(hmm, AA, path), -2.7725887299);
    imm_path_destroy(path);
...
void test_hmm_likelihood_two_states(void)
{
    ...
    imm_float                      lprobs1[] = {logf(0.5f), logf(0.25f), logf(0.5f), logf(1.0)};
    ...
    imm_hmm_set_trans(hmm, imm_normal_state_super(state0), imm_normal_state_super(state0), logf(0.1f));
    imm_hmm_set_trans(hmm, imm_normal_state_super(state0), imm_normal_state_super(state1), logf(0.2f));
    imm_hmm_set_trans(hmm, imm_normal_state_super(state1), imm_normal_state_super(state1), logf(1.0f));
    ...
    path = imm_path_create();
    imm_path_append(path, imm_step_create(imm_normal_state_super(state0), 1));
    cass_cond(is_zero(imm_hmm_loglikelihood(hmm, T, path)));
    imm_path_destroy(path);

    path = imm_path_create();
    imm_path_append(path, imm_step_create(imm_normal_state_super(state1), 1));
    cass_cond(is_zero(imm_hmm_loglikelihood(hmm, G, path)));
    imm_path_destroy(path);
    ...
    path = imm_path_create();
    imm_path_append(path, imm_step_create(imm_normal_state_super(state0), 1));
    cass_close(imm_hmm_loglikelihood(hmm, G, path), -0.6931471825);
    imm_path_destroy(path);

    path = imm_path_create();
    imm_path_append(path, imm_step_create(imm_normal_state_super(state0), 1));
    imm_path_append(path, imm_step_create(imm_normal_state_super(state1), 1));
    cass_close(imm_hmm_loglikelihood(hmm, GT, path), -1.0986122899);
    imm_path_destroy(path);
...
void test_hmm_likelihood_mute_state(void)
{
    ...
    imm_hmm_add_state(hmm, imm_mute_state_super(state), logf(1.0));

    imm_hmm_set_trans(hmm, imm_mute_state_super(state), imm_mute_state_super(state), logf(0.1f));
    ...
    cass_close(imm_hmm_loglikelihood(hmm, EMPTY, path), 0.0f);
    imm_path_destroy(path);
...
void test_hmm_likelihood_two_mute_states(void)
{
    ...
    imm_hmm_add_state(hmm, imm_mute_state_super(S0), 0.0f);
    imm_hmm_add_state(hmm, imm_mute_state_super(S1), zero());

    imm_hmm_set_trans(hmm, imm_mute_state_super(S0), imm_mute_state_super(S1), 0.0f);
...
void test_hmm_likelihood_invalid(void)
{
    ...
    imm_hmm_set_trans(hmm, imm_mute_state_super(S), imm_mute_state_super(M1), 0.0f);
    imm_hmm_set_trans(hmm, imm_mute_state_super(M1), imm_normal_state_super(M2), 0.0f);
    imm_hmm_set_trans(hmm, imm_normal_state_super(M2), imm_mute_state_super(E), 0.0f);
    imm_hmm_set_trans(hmm, imm_mute_state_super(E), imm_mute_state_super(E), 0.0f);
