...
void test_sequence_table(void)
{
    ...
    cass_close(log(0.5), imm_sequence_table_lprob(table, "GG", 2));
    cass_cond(imm_lprob_is_zero(imm_sequence_table_lprob(table, "GGT", 3)));

    imm_sequence_table_add(table, "", log(0.1));
    cass_close(log(0.1), imm_sequence_table_lprob(table, "", 0));

    cass_cond(imm_sequence_table_normalize(table) == 0);

    cass_close(log(0.5 / 0.6), imm_sequence_table_lprob(table, "GG", 2));
    cass_close(log(0.1 / 0.6), imm_sequence_table_lprob(table, "", 0));
