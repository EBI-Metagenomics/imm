...
void test_hmm_viterbi_one_mute_state(void)
{
    ...
    struct imm_path* path = imm_path_create();
    cass_close(imm_hmm_viterbi(hmm, EMPTY, cast_c(state), path), log(0.5));
    ...
    path = imm_path_create();
    cass_cond(!is_valid(imm_hmm_viterbi(hmm, EMPTY, cast_c(state), path)));
    cass_cond(!is_valid(imm_hmm_likelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);
...
void test_hmm_viterbi_two_mute_states(void)
{
    ...
    struct imm_path* path = imm_path_create();
    cass_close(imm_hmm_viterbi(hmm, EMPTY, cast_c(state0), path), log(0.5));
    cass_close(imm_hmm_likelihood(hmm, EMPTY, path), log(0.5));
    imm_path_destroy(path);

    path = imm_path_create();
    cass_close(imm_hmm_viterbi(hmm, EMPTY, cast_c(state1), path), log(0.1));
    ...
    path = imm_path_create();
    cass_cond(!is_valid(imm_hmm_viterbi(hmm, EMPTY, cast_c(state1), path)));
    cass_cond(!is_valid(imm_hmm_likelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);
...
void test_hmm_viterbi_mute_cycle(void)
{
    ...
    struct imm_path* path = imm_path_create();
    cass_cond(!is_valid(imm_hmm_viterbi(hmm, EMPTY, cast_c(state0), path)));
    cass_cond(!is_valid(imm_hmm_likelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);
...
void test_hmm_viterbi_profile1(void)
{
    ...
    struct imm_path* path = imm_path_create();
    cass_close(imm_hmm_viterbi(hmm, EMPTY, cast_c(end), path), log(0.1) + log(1.0));

    cass_close(imm_hmm_likelihood(hmm, EMPTY, path), log(0.1) + log(1.0));
    imm_path_destroy(path);

    path = imm_path_create();
    cass_close(imm_hmm_viterbi(hmm, EMPTY, cast_c(D0), path), log(0.1));
    cass_close(imm_hmm_likelihood(hmm, EMPTY, path), log(0.1));
    imm_path_destroy(path);

    path = imm_path_create();
    cass_close(imm_hmm_viterbi(hmm, EMPTY, cast_c(start), path), log(1.0));
    cass_close(imm_hmm_likelihood(hmm, EMPTY, path), log(1.0));
    imm_path_destroy(path);

    path = imm_path_create();
    cass_cond(!is_valid(imm_hmm_viterbi(hmm, EMPTY, cast_c(M0), path)));
    cass_cond(!is_valid(imm_hmm_likelihood(hmm, EMPTY, path)));
    imm_path_destroy(path);
