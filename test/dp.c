...
void test_dp_illegal(void)
{
    ...
    cass_equal(imm_hmm_set_start(hmm, imm_super(state), imm_log(0.1)),
               IMM_SUCCESS);
    dp = imm_hmm_new_dp(hmm, imm_super(state));
    cass_not_null(dp);
    imm_del(dp);
...
void test_dp_empty_path(void)
{
    ...
    struct imm_seq const *seq = imm_seq_new(1, "A", abc);
...
void test_dp_one_mute(void)
{
    ...
    cass_equal(imm_hmm_set_start(hmm, imm_super(state), imm_log(0.3)),
               IMM_SUCCESS);
    struct imm_dp *dp = imm_hmm_new_dp(hmm, imm_super(state));
...
void test_dp_two_mutes(void)
{
    ...
    cass_equal(imm_path_step(&result.path, 1)->state_id, 12);
    imm_del(seq);
